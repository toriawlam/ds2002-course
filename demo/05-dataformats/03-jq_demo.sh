...
curl -s $DOGS | jq -r ".data"

echo "Get the length of the data array"
curl -s $DOGS | jq -r ".data | length"

echo "get subset of records in array (by index)"
curl -s $DOGS | jq -r ".data[0,2,-1]"

echo "Constructs a new JSON object with selected fields"
curl -s $DOGS | jq -r ".data[] | {name:.attributes.name, hypoallergenic:.attributes.hypoallergenic, life_max:.attributes.life.max}"
